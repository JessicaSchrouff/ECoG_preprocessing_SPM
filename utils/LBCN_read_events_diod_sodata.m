...
try
    ...
    deftask = struct('skip_before',[],'skip_after',[],'thresh_dur',0,...
        'listcond',{});
end
...
for si = 1:lsi
    try
        ...
        if isfield(K,'stimInf') && isfield(K.stimInf,'name')
            stim_Name(si) = K.stimInf(si).name;
            stim_List(si) = K.stimInf(si).list;
        end
...
goodtrials = SOT>0;
SOT = SOT(goodtrials);
if isfield(K.stimInf,'duration')
    dur = [K.stimInf(goodtrials).duration];
end
if isfield(K.stimInf(1),'oneback')
...
[evt] = get_events_diod(fullfile(D.path,D.fname), [], deftask.skip_before, ...
    deftask.skip_after, deftask.thresh_dur);
